...
export default function Navbar({ onOpenBooking }) {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <a href="#home" className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] tracking-widest text-teal-700 font-bold uppercase -mt-1">
              {t('nav.tagline')}
            </span>
          </div>
        </a>
        ...
        <nav className="hidden lg:flex items-center gap-7">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-xs sm:text-sm font-bold text-slate-700 hover:text-teal-600 transition-colors relative py-1"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-xs font-bold text-slate-800 hover:border-teal-500 transition-all cursor-pointer shadow-sm"
          >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="sm:hidden bg-white/98 backdrop-blur-2xl border-b border-slate-200 px-6 py-6 shadow-2xl">
          <div className="flex flex-col gap-4">
            {navLinks.map((link) => (
              <a
                ...
                className="text-base font-bold text-slate-800 hover:text-teal-600 py-1 border-b border-slate-100"
              >
                {link.name}
              </a>
            ))}
            ...
            <div className="flex items-center justify-between pt-3">
              <button
                onClick={toggleLanguage}
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 text-sm font-bold text-slate-800"
              >
                <Globe2 className="w-4 h-4 text-teal-600" />
                <span>{t('nav.lang')}</span>
              </button>
